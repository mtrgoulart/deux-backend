...
class OKXInterface(ExchangeInterface):
    ...
    def place_order(self, symbol, side, order_type, size, currency, price=None):
        general_logger.debug(f"Placing order: {symbol}, {side}, {order_type}, {size}, {currency}, {price}")
    ...
    def get_fill_price(self, order_id):
        general_logger.debug(f"Getting fill price for order ID: {order_id}")
        return self.okx_client.wait_for_fill_price(order_id)

    def cancel_order(self, symbol, order):
        general_logger.debug(f"Cancelling order: {order} for symbol: {symbol}")
        response = self.okx_client.cancel_order(symbol, order)
        return response

    def get_open_order(self, symbol):
        general_logger.debug(f"Getting open orders for symbol: {symbol}")
    ...
    def get_last_trade(self, symbol):
        general_logger.debug(f"Getting last trade for symbol: {symbol}")
        response = self.okx_client.get_last_trade(symbol)
        return response

    def get_balance(self, ccy=None):
        general_logger.debug(f"Getting balance for currency: {ccy}")
    ...
    def get_order_execution_price(self, symbol, order_id):
        general_logger.debug(f"Getting execution price for order ID: {order_id} on symbol: {symbol}")
