    ...
    def start_operation(self):
        ...
        try:
            ...
            if key not in self.saved_data:
                raise ValueError(f"Não existem dados salvos para o par {symbol} .")
...
class DynamicForm(ctk.CTkFrame):
    ...
    def add_new_data_set(self):
        ...
        # Frame Pai para o conjunto de dados
        data_set_frame = ctk.CTkFrame(self.scrollable_frame)
        data_set_frame.pack(pady=5, padx=5, fill="x")

        # Campos do Frame Pai (incluindo 'interval' e '% Operações')
        parent_entries = self.create_data_entries(data_set_frame, [["symbol", "interval", "% Operações"]])

        # Frame Filho para "buy"
        buy_frame = ctk.CTkFrame(data_set_frame)
        buy_frame.pack(pady=5, padx=5, fill="x")
        buy_entries = self.create_data_entries(buy_frame, [["side", "condition"]])
        buy_entries["side"].insert(0, "buy")  # Definindo o valor padrão "buy" para o side

        # Frame Filho para "sell"
        sell_frame = ctk.CTkFrame(data_set_frame)
        sell_frame.pack(pady=5, padx=5, fill="x")
        sell_entries = self.create_data_entries(sell_frame, [["side", "condition"]])
        sell_entries["side"].insert(0, "sell")  # Definindo o valor padrão "sell" para o side

        # Criar ButtonManager
        ButtonManager(parent_entries, img_path, self.saved_data, self.app_instance, data_set_frame)
        self.data_entries.append({
            "parent_entries": parent_entries,
            "buy_entries": buy_entries,
            "sell_entries": sell_entries,
            "frame": data_set_frame
        })
